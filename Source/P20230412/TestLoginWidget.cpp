...
void UTestLoginWidget::OnCreateSignUpButtonClicked()
{
    ...
    RemoveFromParent();
    //위젯 생성하기(
    FSoftClassPath testwidget(TEXT("/Script/UMGEditor.WidgetBlueprint'/Game/Singupwidget.Singupwidget_C'"));
    UClass* widgetClass = testwidget.TryLoadClass<UTestLoginWidget>();
    UTestLoginWidget* widgetObject = CreateWidget<UTestLoginWidget>(this, widgetClass);
...
void UTestLoginWidget::OnBackButtonClicked()
{
    ...
    //"/Game/LoginWidget.LoginWidget_C" 경로에 있는 WidgetBlueprint를 로드하고, 
    //이를 기반으로 UTestLoginWidget 클래스의 위젯 객체를 생성하여 현재 컨텍스트에 추가하는 역할
    FSoftClassPath testwidget(TEXT("/Script/UMGEditor.WidgetBlueprint'/Game/LoginWidget.LoginWidget_C'"));
    UClass* widgetClass = testwidget.TryLoadClass<UTestLoginWidget>();
    UTestLoginWidget* widgetObject = CreateWidget<UTestLoginWidget>(this, widgetClass);

    if (widgetObject)
    {
        widgetObject->AddToViewport();  //화면에 보여줌
    }
}
